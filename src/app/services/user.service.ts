...
export class UserService {
  ...
  logInUser(userInfo:UserLogin):any {
    ...
    return this.http
    .post<UserLogin[]>(this.path + '/auth/login', body)
    .pipe(
      tap(data => {     
      ...
      catchError(err => {
        err['status']===400 ? this.alertifyService.error('Invalid Email') : null 
        return err
      })
    )
  }
  ...
  updateUserInfos(userUpdate:UserInfos): Observable<UserInfos[]> {
    ...
    const body = {
      ...
      'about':userUpdate.about ==  null || userUpdate.about == ""  ?  undefined: userUpdate.about
    }
  ...
  resetPassword(newPassword:Password, resetPasswordToken:string): Observable<Password[]> {
    return this.http
    .put<Password[]>(this.path + '/auth/resetpassword?resetPasswordToken=' + resetPasswordToken, newPassword)
    .pipe(
      tap( data => {
        console.log(JSON.stringify(data))
      },
